...
class AwsProvider {
   ...
   */
  request(service, method, params, useCache) {
    const that = this;
    ...
    if (shouldCache) {
      ...
      const cachedResult = _.get(this.requestCache, `${service}.${method}.${paramsHash}`);
      if (cachedResult) {
        return BbPromise.resolve(cachedResult);
      }
    }
      ...
      return new BbPromise((resolve, reject) => {
        req.send((errParam, data) => {
          ...
          if (err) {
            ...
            if (shouldCache) {
              const paramsHash = objectHash.sha1(params);
              _.set(this.requestCache, `${service}.${method}.${paramsHash}`, data);
            }
            resolve(data);
          }
        });
